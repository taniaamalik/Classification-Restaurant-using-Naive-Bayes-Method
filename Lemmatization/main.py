import testingMultinomialLemmatization as tsML
import trainingMultinomialLemmatization as trML
import gaussianLemmatization as gausL
...
def gaussian(listTraining, listTesting):
    hasilBenar = gausL.gaussian(listTraining,listTesting)
...
def multinomial(listTraining, listTesting):
    trainingReturnValue = trML.training(listTraining)
    ...
    for indexKomentar in range(len(listTesting)):
        ...
        if indexKomentar % 2 == 0:
            positif = True
        returnvalue = tsML.testing(listTesting[indexKomentar], myConProb, termsMain, positif, inputKomentar)
